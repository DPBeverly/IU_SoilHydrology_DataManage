...
##//Where is the data
setwd("D:/Dropbox/Projects/Indiana/SoilLab/Data/KSat/RawData/LabTesting")
dir()
...
LabCalibration_Ksat <- Fdat
##//Take in a look
cc <- Fdat$mymaterials=="5050" & Fdat$Fitting_cm>0 & Fdat$FittingParameter_r2>=0.999 & Fdat$myreps!="r2"
...
Fdat_sum1 <- Fdat[Fdat$Fitting_cm>0 & Fdat$FittingParameter_r2>=0.999 & Fdat$myreps!="r2",] %>%
  #dplyr::mutate(day = as.Date(Date_ct, format="%d-%m-%Y")) %>%
  dplyr::group_by(mymaterials) %>% # group by the day column ##//mean looks good
  dplyr::summarise_if(is.numeric, median, na.rm = TRUE) %>%  
  na.omit

Fdat_sd <- Fdat[Fdat$Fitting_cm>0 & Fdat$FittingParameter_r2>=0.999 & Fdat$myreps!="r2",] %>%
  #dplyr::mutate(day = as.Date(Date_ct, format="%d-%m-%Y")) %>%
  dplyr::group_by(mymaterials) %>% # group by the day column ##//mean looks good
...
LabCal_DF <- data.frame(Material = c("5050", "Organic", "Sand"),
...
mysubs <- unique(LabCal_DF$Material)

MyTimes <- seq(0,500, by = 1)
Labdouts <- c() 
i = 1
for(i in 1:length(mysubs)){
  Pressure <- LabCal_DF$a_param[LabCal_DF$Material==mysubs[i]] * 
    exp(LabCal_DF$b_param[LabCal_DF$Material==mysubs[i]] * 
          MyTimes)
  LPressure <- LabCal_DF$a_param_LCI[LabCal_DF$Material==mysubs[i]] * 
    exp(LabCal_DF$b_param_LCI[LabCal_DF$Material==mysubs[i]] * 
          MyTimes)
  HPressure <- LabCal_DF$a_param_HCI[LabCal_DF$Material==mysubs[i]] * 
    exp(LabCal_DF$b_param_HCI[LabCal_DF$Material==mysubs[i]] *
          MyTimes)
  
  Labdouts <- rbind(Labdouts, data.frame(Material = mysubs[i],
                                   Time = MyTimes, 
                                   Mean_Flow = Pressure,
                                   Flow_LCI = LPressure,
                                   Flow_HCI = HPressure))
...
plot(Labdouts$Time, Labdouts$Mean_Flow, ylim = c(0,7))
lines(Labdouts$Time, Labdouts$Flow_LCI, lwd = 2)
lines(Labdouts$Time, Labdouts$Flow_HCI, lwd = 2, col = "blue")
